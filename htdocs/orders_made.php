...
$conn = Utility\get_a_connection();
$sql = $conn->prepare("select * from `order` where client_id_of_buyer = ?");
$sql->bind_param("s", $_SESSION["client_id"]);
$sql->execute();
...
while ($row = $result->fetch_assoc()) {
    ...
    $sub_sql2->bind_param("s", $row['client_id_of_seller']);
    $sub_sql2->execute();
    ...
    echo '<p style = "font-size:15px;position: relative; left: 30px;">Seller Username:' . $sub_result2['username'] . '</p>';
    echo '<p style = "font-size:15px;position: relative; left: 30px;">Seller Phone Number:' . $sub_result2['phone_number'] . '</p>';
